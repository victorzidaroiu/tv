...
angular.module('tvApp', ['ngRoute', 'ngCookies']).config(['$routeProvider', function ($routeProvider) {
	$routeProvider.when('/', {
				...
				$http.get('/api/catalogue/' + _this.locationID).success(function (response) {
					if (response.error === false) {
						...
						for (var productID in _this.catalogue) {
							if (_this.basket[productID] !== undefined) _this.catalogue[productID].isInBasket = true;else _this.catalogue[productID].isInBasket = false;
						}
						$timeout(function () {
	...
	if (!_this.customerID) $http.post('/api/customer', {
	...
	if (!_this.customerID) _this.createCustomerID();else _this.getLocation();
