...
"use strict";angular.module("tvApp",["ngRoute","ngCookies"]).config(["$routeProvider",function(a){a.when("/",{templateUrl:"partials/index.html",controller:"indexController",title:"Product selection"}).when("/checkout",{templateUrl:"partials/checkout.html",controller:"checkoutController",title:"Product checkout"}).otherwise({redirectTo:"/"})}]).controller("checkoutController",["$cookies","$scope","$http",function(a,b,c){var d=this;d.list=[],b.orders=d,d.get=function(){c.get("/api/top-orders").success(function(a){d.list=a.map(function(a){return a.orderId=a._id,delete a._id,a})})},d.get()}]).controller("indexController",["$cookies","$scope","$http",function(a,b,c){var d=this,e=["LONDON","LIVERPOOL"];d.list=[],b.products=d,console.log(a);var f=a.get("customerID");f?d.getLocation():c.post("/api/customer",{locationID:e[Date.now()%2===0?0:1]}).success(function(a){a.error===!1&&(f=a.data.customerID,d.getLocation())}),d.getLocation=function(){c.get("/api/customer-location/"+f).success(function(a){a.error===!1&&(locationID=a.data.locationID,c.get("/api/catalogue/"+locationID).success(function(a){a.error===!1&&(b.catalogue=a.data)}))})}}]).run(["$rootScope",function(a){a.$on("$routeChangeSuccess",function(b,c,d){c.$$route&&(a.title=c.$$route.title)})}]),String.prototype.trim||(String.prototype.trim=function(){return this.replace(/^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g,"")});