...
"use strict";angular.module("tvApp",["ngRoute","ngCookies"]).config(["$routeProvider",function(a){a.when("/",{templateUrl:"partials/index.html",controller:"indexController",title:"Product selection"}).when("/checkout",{templateUrl:"partials/checkout.html",controller:"checkoutController",title:"Product checkout"}).otherwise({redirectTo:"/"})}]).controller("checkoutController",["$timeout","$cookies","$scope","$http",function(a,b,c,d){var e=this;e.basket={},e.catalogue=[],c.products=e,e.customerID=b.get("customerID"),e.locationID=null,e.openAddModal=function(){$("#orderConfirmModal").modal("show")},e.getLocation=function(){d.get("/api/customer-location/"+e.customerID).success(function(b){b.error===!1&&(e.locationID=b.data.locationID,e.basket=b.data.basket,d.get("/api/catalogue/"+e.locationID).success(function(b){if(b.error===!1){e.catalogue=b.data;for(var c in e.catalogue)void 0!==e.basket[c]?e.catalogue[c].isInBasket=!0:e.catalogue[c].isInBasket=!1;a(function(){$(".ui.checkbox").checkbox({onChange:e.updateBasket})},0,!1)}}))})},e.updateBasket=function(){e.basket={},$("input:checked").each(function(){e.basket[$(this).data("id")]=!0}),d.put("/api/customer/"+e.customerID,{basket:e.basket}),c.$apply()},e.customerID?e.getLocation():d.post("/api/customer",{locationID:locations[Date.now()%2===0?0:1],basket:{}}).success(function(a){a.error===!1&&(e.customerID=a.data.customerID,b.put("customerID",e.customerID),e.getLocation())})}]).controller("indexController",["$timeout","$cookies","$scope","$http","$log",function(a,b,c,d,e){var f=this,g=["LONDON","LIVERPOOL"];f.basket={},f.catalogue=[],c.products=f,c.$log=e,f.basketSize=0,f.customerID=b.get("customerID"),f.locationID=null,f.getLocation=function(){d.get("/api/customer-location/"+f.customerID).success(function(b){b.error===!1?(f.locationID=b.data.locationID,f.basket=b.data.basket,d.get("/api/catalogue/"+f.locationID).success(function(b){if(b.error===!1){f.catalogue=b.data;for(var c in f.catalogue)void 0!==f.basket[c]?f.catalogue[c].isInBasket=!0:f.catalogue[c].isInBasket=!1;f.basketSize=Object.keys(f.basket).length,a(function(){$(".ui.checkbox").checkbox({onChange:f.updateBasket})},0,!1)}})):f.createCustomerID()})},f.updateBasket=function(){f.basket={},$("input:checked").each(function(){f.basket[$(this).data("id")]=!0}),d.put("/api/customer/"+f.customerID,{basket:f.basket}),f.basketSize=Object.keys(f.basket).length,c.$apply()},f.createCustomerID=function(){d.post("/api/customer",{locationID:g[Date.now()%2===0?0:1],basket:{}}).success(function(a){a.error===!1&&(f.customerID=a.data.customerID,b.put("customerID",f.customerID),f.getLocation())})},f.customerID?f.getLocation():f.createCustomerID()}]).run(["$rootScope",function(a){a.$on("$routeChangeSuccess",function(b,c,d){c.$$route&&(a.title=c.$$route.title)})}]),void 0===String.prototype.trim&&(String.prototype.trim=function(){return this.replace(/^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g,"")});